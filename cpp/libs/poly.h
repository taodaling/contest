...
void mul(vector<T>& a, const vector<T>& b) {
    ...
    int t = 4 * n;
    for (size_t i = 0; i < n; i++) {
        int64_t A0 = llroundl(real(C[i]) / t);
        T A1 = llroundl(imag(D[i]) / t);
        T A2 = llroundl(imag(C[i]) / t);
...
struct poly {
    ...
    vector<T> eval(vector<poly>& tree,
                   int v,
                   iter l,
...
poly<T> build(vector<poly<T>>& res,
              int v,
              iter L,
