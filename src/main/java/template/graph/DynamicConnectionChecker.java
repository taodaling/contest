...
public class DynamicConnectionChecker {
    ...
    public DynamicConnectionChecker(int n, int m) {
        ...
        Buffer<Splay> deque = new Buffer<>(Splay::new);
        for (int i = 1; i <= log2; i++) {
            levels[i] = new EulerTourTree(n, deque);
            levels[i].id = i;
        }
        map = new LongObjectHashMap<>(m, false);
    }
    ...
    public static class EulerTourTree {
        ...
        public EulerTourTree(int n) {
            this(n, new Buffer<>(Splay::new));
        }

        public EulerTourTree(int n, Buffer<Splay> buffer) {
            this.buffer = buffer;
