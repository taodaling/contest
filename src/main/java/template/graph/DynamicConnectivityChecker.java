...
public class DynamicConnectivityChecker {
    EulerTourTree[] levels;
    int n;
    int log2;
    LongObjectHashMap<EdgeInfo> map;

    public boolean containEdge(int i, int j) {
        return map.containKey(idOfEdge(i, j));
    }

    public int getConnectedComponentSize(int i) {
        Splay node = levels[levels.length - 1].nodes[i];
        Splay.splay(node);
        return node.size;
    }
