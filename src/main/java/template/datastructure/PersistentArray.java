...
public class PersistentArray<T> {
    ...
    private PersistentArray(PersistentTreap data) {
        this.data = data;
    }
     ...
     */
    public PersistentArray<T> insert(int i, T val) {
        ...
        PersistentArray<T> ans = new PersistentArray<>(PersistentTreap.merge(split[0], split[1]));
        return ans;
    }
     ...
     */
    public PersistentArray<T> interval(int l, int r) {
        int size = size();
        if (l == 1 && r == size) {
            return this;
        }
        ...
        if (l == 1) {
            ...
            return new PersistentArray<>(split[0]);
        }
        if (r == size) {
            PersistentTreap[] split = PersistentTreap.splitByRank(data, l - 1);
            return new PersistentArray<>(split[1]);
        }
        PersistentTreap[] split = PersistentTreap.splitByRank(data, r);
        return new PersistentArray<>(PersistentTreap.splitByRank(split[0], l - 1)[1]);
    }

    /**
     * Split this array into 1,2,3,...,mid and mid+1,...,size
     */
    public PersistentArray<T>[] split(int mid) {
        PersistentTreap[] split = PersistentTreap.splitByRank(data, mid);
        return new PersistentArray[]{new PersistentArray(split[0]), new PersistentArray(split[1])};
    }

    public PersistentArray<T> merge(PersistentArray<T> other) {
        return new PersistentArray<>(PersistentTreap.merge(data, other.data));
    }

    public PersistentArray<T> modify(int i, T val) {
        PersistentTreap[] split = PersistentTreap.splitByRank(data, i - 1);
        ...
        return new PersistentArray<>(PersistentTreap.merge(split[0], split[1]));
    }
