...
public class UndirectedModifiableMinDistProblem {
    ...
    public long queryOnAddEdge(int a, int b, long dist) {
        ...
        ans = Math.min(ans, nodes[b].distToSrc + dist + nodes[a].distToDst);
        return ans;
    }
    ...
    public UndirectedModifiableMinDistProblem(int n, int m, int s, int t) {
        nodes = new Node[n];
        for (int i = 0; i < n; i++) {
            nodes[i] = new Node();
            nodes[i].id = i;
