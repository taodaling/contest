...
public class LongSparseTable2D {
    private ArrayIndex ai;
    private long[] data;
    private LongBinaryFunction merger;

    public LongSparseTable2D(Int2ToLongFunction mat, int n, int m, LongBinaryFunction merger) {
        this.merger = merger;
        int logn = Log2.floorLog(n);
        int logm = Log2.floorLog(m);
        ai = new ArrayIndex(logn + 1, n, logm + 1, m);
        data = new long[ai.totalSize()];
                ...
                for (int k = 0; k <= logm; k++) {
                    for (int t = 0; t < m; t++) {
                        if (i == 0 && k == 0) {
                            ...
                            long ans = merger.apply(data[ai.indexOf(i - 1, j, k, t)],
                                    data[ai.indexOf(i - 1, Math.min(j + (1 << i - 1), n - 1), k, t)]);
    ...
    public long query(int b, int t, int l, int r) {
        int h = t - b + 1;
        int w = r - l + 1;
        int logh = Log2.floorLog(h);
        int logw = Log2.floorLog(w);

        long ans = merger.apply(data[ai.indexOf(logh, b, logw, l)],
