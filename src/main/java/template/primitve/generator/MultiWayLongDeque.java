...
public class MultiWayLongDeque {
    private long[] values;
    private int[] next;
    private int[] prev;
    private int[] heads;
    ...
    public LongDeque getDeque(int qId) {
        return new LongDeque() {
            ...
            public void addFirst(long x) {
                MultiWayLongDeque.this.addFirst(qId, x);
            }

            @Override
            public long removeFirst() {
                return MultiWayLongDeque.this.removeFirst(qId);
            }

            @Override
            public long peekFirst() {
                return MultiWayLongDeque.this.peekFirst(qId);
            }

            @Override
            public void addLast(long x) {
                MultiWayLongDeque.this.addLast(qId, x);
            }

            @Override
            public long removeLast() {
                return MultiWayLongDeque.this.removeLast(qId);
            }

            @Override
            public long peekLast() {
                return MultiWayLongDeque.this.peekLast(qId);
            }

            @Override
            public LongIterator iterator() {
                return MultiWayLongDeque.this.iterator(qId);
            }

            @Override
            public boolean isEmpty() {
                return MultiWayLongDeque.this.isEmpty(qId);
            }
        };
    }
    ...
    public MultiWayLongDeque(int qNum, int totalCapacity) {
