...
public class DoubleSparseTable2D {
    private ArrayIndex ai;
    private double[] data;
    private DoubleBinaryFunction merger;

    public DoubleSparseTable2D(Int2ToDoubleFunction mat, int n, int m, DoubleBinaryFunction merger) {
        this.merger = merger;
                ...
                for (int k = 0; k <= logm; k++) {
                    for (int t = 0; t < m; t++) {
                        if (i == 0 && k == 0) {
                            ...
                            double ans = merger.apply(data[ai.indexOf(i - 1, j, k, t)],
                                    data[ai.indexOf(i - 1, Math.min(j + (1 << i - 1), n - 1), k, t)]);
    ...
    public double query(int b, int t, int l, int r) {
        int h = t - b + 1;
        int w = r - l + 1;
        int logh = Log2.floorLog(h);
        int logw = Log2.floorLog(w);

        double ans = merger.apply(data[ai.indexOf(logh, b, logw, l)],
