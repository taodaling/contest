...
public class DoubleBIT2DExt {
    ...
    public DoubleBIT2DExt(int n, int m) {
        this.n = n;
        this.m = m;
        delta = new double[n + 1][m + 1];
        idelta = new double[n + 1][m + 1];
        jdelta = new double[n + 1][m + 1];
        ijdelta = new double[n + 1][m + 1];
    }
    ...
    private void update(int x, int y, double mod) {
        double x1 = mod;
        double x2 = mod * x;
        double x3 = mod * y;
        double x4 = mod * x * y;
    ...
    public void update(int ltx, int lty, int rbx, int rby, double mod) {
        update(ltx, lty, mod);
    ...
    public double query(int ltx, int lty, int rbx, int rby) {
        double ans = query(rbx, rby);
        if (ltx > 1) {
            ans -= query(ltx - 1, rby);
        }
        if (lty > 1) {
