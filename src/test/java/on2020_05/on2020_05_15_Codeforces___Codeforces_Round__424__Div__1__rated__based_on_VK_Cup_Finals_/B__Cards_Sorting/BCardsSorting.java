package on2020_05.on2020_05_15_Codeforces___Codeforces_Round__424__Div__1__rated__based_on_VK_Cup_Finals_.B__Cards_Sorting;
...
public class BCardsSorting {
    public void solve(int testNumber, FastInput in, FastOutput out) {
        int n = in.readInt();
        Splay[] nodes = new Splay[n];
        for (int i = 0; i < n; i++) {
            nodes[i] = new Splay();
            nodes[i].key = in.readInt();
            if (i > 0) {
        ...
        long ans = 0;
        Arrays.sort(nodes, (a, b) -> Integer.compare(a.key, b.key));
        for (int i = 0; i < n; i++) {
            int l = i;
            int r = i;
            while (r + 1 < n && nodes[r + 1].key == nodes[l].key) {
                r++;
            }
            i = r;
            for (int j = l; j <= r; j++) {
                Splay.splay(nodes[j]);
                nodes[j].rank = nodes[j].left.size;
            }
            Arrays.sort(nodes, l, r + 1, (a, b) -> Integer.compare(a.rank, b.rank));
            for (int j = l; j <= r; j++) {
                Splay.splay(nodes[j]);
                Splay left = Splay.splitLeft(nodes[j]);
                Splay right = Splay.splitRight(nodes[j]);
                ans += left.size + 1;
                Splay.merge(right, left);
            }
        }
