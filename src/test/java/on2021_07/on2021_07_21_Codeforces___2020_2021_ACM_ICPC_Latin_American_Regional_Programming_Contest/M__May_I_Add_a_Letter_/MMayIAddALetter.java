...
public class MMayIAddALetter {
    ...
    public void solve(int testNumber, FastInput in, FastOutput out) {
        ...
        int n = in.rs(s);
        int m = in.rs(q);
        SuffixBalancedTreeLcp lcp = new SuffixBalancedTreeLcp(n + m);
        for (int i = 0; i < n; i++) {
            lcp.addPrefix(s[i]);
        }
        debug.debug("lcp", lcp);
        out.println(lcp.root.deltaSum);
        for (int i = 0; i < m; i++) {
            if (q[i] == '-') {
                lcp.removePrefix();
            } else {
...
//        root.pushUp();
    ...
    public static class Node implements Cloneable, Comparable<Node> {
        ...
        public void pushUp() {
            ...
            deltaSum = left.deltaSum + right.deltaSum;
            if (occur > 0) {
                rangeMinLCP = Math.min(rangeMinLCP, lcp);
                deltaSum += Math.max(0, lcp - prev.lcp);
            }
        }
