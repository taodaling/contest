package on2021_01.on2021_01_13_CSES___CSES_Problem_Set.Reversal_Sorting;
...
public class ReversalSorting {
    ...
    public void solve(int testNumber, FastInput in, FastOutput out) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            Splay.splay(node);
            node.pushDown();
            int rank = node.size - node.right.size;
            ans.add(new int[]{i + 1, i + rank});


            // debug.debug("root", Splay.cloneTree(node));

            Splay right = Splay.splitRight(node);
            Splay left = Splay.splitLeft(node);
            left.reverse();
