package on2019_12.on2019_12_30_Good_Bye_2019.F__Awesome_Substrings;
...
public class FAwesomeSubstrings {
    public void solve(int testNumber, FastInput in, FastOutput out) {
        ...
        int blockSize = 4000;//(int) Math.ceil(Math.sqrt(n));
...
//        }
        ...
        IntegerHashMap[] cnts = new IntegerHashMap[invBlockSize + 1];
        for (int i = 1; i <= invBlockSize; i++) {
            cnts[i] = new IntegerHashMap(n, false);
        }
        for (int i = 0, j = 0; i < n; i++) {
            while (ps.prefix(i) - ps.prefix(j - 1) >= blockSize) {
                ...
                for (int k = 1; k <= invBlockSize; k++) {
                    int v = (j - 1) - k * sl;
                    cnts[k].put(v, cnts[k].getOrDefault(v, 0) + 1);
                }
