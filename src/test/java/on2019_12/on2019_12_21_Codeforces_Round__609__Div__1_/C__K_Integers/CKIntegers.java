package on2019_12.on2019_12_21_Codeforces_Round__609__Div__1_.C__K_Integers;
...
public class CKIntegers {
    public void solve(int testNumber, FastInput in, FastOutput out) {
        ...
        for (int i = 2; i <= n; i++) {
            ...
            newNode.key = indexes[i];
            newNode.pushUp();
            root = Splay.add(root, newNode);
            Splay.splay(newNode);
            inv += newNode.right.size;

            root = Splay.selectKthAsRoot(root, DigitUtils.ceilDiv(i, 2));
            long dist = sumOf(root.key - root.left.size, root.key - 1) - root.left.sum
                    + root.right.sum - sumOf(root.key + 1, root.key + root.right.size);

            out.println(dist + inv);
        }
    }

    public long sumOf(long l, long r){
        if(r < l){
            return 0;
        }
        return (l + r) * (r - l + 1) / 2;
