package on2019_12.on2019_12_18_Codeforces_Global_Round_6.F__Almost_Same_Distance0;
...
public class FAlmostSameDistance {
    ...
    public void solve(int testNumber, FastInput in, FastOutput out) {
        ...
        type2(nodes[1], null);
        for (int i = n - 1; i >= 1; i--) {
            ans1[i] = Math.max(ans1[i], ans1[i + 1]);
        }
        for (int i = n - 2; i >= 1; i--) {
            if (i + 2 <= n) {
                ans2[i] = Math.max(ans2[i], ans2[i + 2]);
            }
        }
        for (int i = 1; i <= n; i++) {
            out.println(Math.max(ans1[i], ans2[i]));
        }
    }
    ...
    public void type2(Node root, Node p) {
        ...
        for (int i = root.dists.length - 1; i >= 0; i--) {
            ...
            if (cur <= distInFather) {
                index = Math.min(index + 1, p.dists.length);
            }
            int cnt = root.dists.length - i - encounter + p.dists.length - index;
            if (cnt > 1) {
                ans2[2 * cur] = Math.max(ans2[2 * cur], cnt);
            }
        }
    }
    ...
    public void type1(Node root, Node p) {
        ...
        for (int i = m - 1; i >= 0; i--) {
            ...
            int cnt = m - i;
            if (cnt > 1) {
                ans1[cur * 2] = Math.max(ans1[cur * 2], cnt);
            }
            if (i + 1 < m && list.get(i + 1) > cur) {
                ans1[cur * 2 + 1] = Math.max(ans1[cur * 2 + 1], cnt);
            }
        }
