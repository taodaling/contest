...
public class TopRow {
    public void solve(int testNumber, FastInput in, FastOutput out) {
        ...
        int L = 0;
        int R = n - 1;
        PersistentSegment[] ps = new PersistentSegment[n + 1];
...
class PersistentSegment implements Cloneable {
    ...
    void modify(long ma, long mb, int l, int r) {
        assert this != NIL;
        if(this == NIL){
            return;
        }
        a += ma;
        b += mb;
        sum += ma * IntMath.sumOfInterval(l, r) + mb * (r - l + 1);
    }
...
//        }
    ...
    public void update(int ll, int rr, int l, int r, long ma, long mb) {
        assert this != NIL;
        ...
        pushDown(l, r);
        int m = DigitUtils.floorAverage(l, r);
        left.update(ll, rr, l, m, ma, mb);
        right.update(ll, rr, m + 1, r, ma, mb);
        pushUp();
    }
    ...
    public long query(int ll, int rr, int l, int r) {
        ...
        if(this.left == NIL){
            l = Math.max(l, ll);
            r = Math.min(r, rr);
            return a * IntMath.sumOfInterval(l, r) + b * (r - l + 1);
        }
        pushDown(l, r);
        int m = DigitUtils.floorAverage(l, r);
...
class LCTNode {
    ...
    static void consider(LCTNode node, int mod) {
        ...
        persistentSegment.update(l, r, L, R, -1 * mod, (1 + i + 1) * mod);
    }
