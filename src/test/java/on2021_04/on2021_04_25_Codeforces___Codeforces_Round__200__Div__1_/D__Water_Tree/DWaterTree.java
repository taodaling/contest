package on2021_04.on2021_04_25_Codeforces___Codeforces_Round__200__Div__1_.D__Water_Tree;
...
public class DWaterTree {
    public void solve(int testNumber, FastInput in, FastOutput out) {
        int n = in.ri();
        HeavyLightDecompose hld = new HeavyLightDecompose(Long::sum, n, 0);
        for (int i = 0; i < n - 1; i++) {
            int a = in.ri() - 1;
            int b = in.ri() - 1;
            hld.addEdge(a, b);
        ...
        for (int i = 0; i < q; i++) {
            ...
            int v = in.ri() - 1;
            if (c == 1) {
                hld.updateSubtree(v, 1);
            } else if (c == 2) {
                hld.updatePath(v, 0, 0);
            } else {
                long ans = hld.processPath(v, v);
                out.println(ans);
            }
        }
    }
