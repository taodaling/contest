package on2021_05.on2021_05_19_DMOJ.DMOPC__20_Contest_5_P6___Top_Row;
...
class LazyPersistentSegTreeBasedOnArray {
    ...
    public void query(int L, int R, Sum s) {
        query(last, L, R, s);
    }

    public void query(int version, int L, int R, Sum s) {
        query0(clone(version), L, R, begin, end, s);
    }

    private void query0(int root, int L, int R, int l, int r, Sum s) {
        ...
        pushDown(root, l, r);
        query0(left[root], L, R, l, m, s);
        query0(right[root], L, R, m + 1, r, s);
        return;
    }
